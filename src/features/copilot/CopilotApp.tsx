...
import { registerComposerTool } from './webmcp';
import type {
  Conversation,
  Role,
  Locale,
...
export default function CopilotApp({
  ...
  useEffect(() => registerComposerTool(setInput), []);
  useEffect(() => {
    let mounted = true;
    void (async () => {
      try {
        ...
        setCurrent(
          draftId
            ? (all.find((c) => c.draft?.id === draftId) ?? null)
            : (all[0] ?? null),
        );
      } catch (e) {
  ...
  async function refresh() {
    const all = await api<Conversation[]>('conversations');
    setConversations(all);
    if (current) setCurrent(all.find((c) => c.id === current.id) ?? current);
  }
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      const r = await fetch('/api/copilot/files', {
        method: 'POST',
        body: form,
      });
